	...
	for _, ticket := range tickets {
		...
		case ticket.Offer.Auto:
			...
			case err == clientdb.ErrAccountNotFound:
				err := a.AutoAcceptSidecar(
					context.Background(), ticket,
				)
				if err != nil {
			...
			case err == nil:
				...
				err = a.CoordinateSidecar(
					context.Background(), ticket, ticketBid,
					acct,
				)
				if err != nil {
					return fmt.Errorf("unable to resume "+
...
func (a *SidecarAcceptor) deriveProviderStreamID(ticket *sidecar.Ticket,
) ([64]byte, error) {
...
func (a *SidecarAcceptor) deriveRecipientStreamID(ticket *sidecar.Ticket) [64]byte {
	var streamID [64]byte
	copy(streamID[:], ticket.Recipient.NodePubKey.SerializeCompressed())
	copy(streamID[:], ticket.Recipient.MultiSigPubKey.SerializeCompressed())

	return streamID
}

// validateOfferedTicket validates a ticket in the offered state to ensure all the
func validateOfferedTicket(ctx context.Context, t *sidecar.Ticket,
	signer lndclient.SignerClient, db sidecar.Store) error {

	// The ticket should be in the offered state at
	// this point (has the bid information).
	if t.State != sidecar.StateOffered {
		return fmt.Errorf("sidecar ticket in state %v, expected %v",
			t.State, sidecar.StateRegistered)
	}
...
func (a *SidecarAcceptor) AutoAcceptSidecar(ctx context.Context,
	...
	go func() {
		...
		for {
			...
			// the ticket w/o our node information, we'll populate
			// it then send it to them over the cipherbox they've
			...
			case sidecar.StateOffered:
				...
				log.Infof("Registering ticket %x", ticket.ID[:])

				// We'll now register the ticket as normal then
				// send it over to the provider and await their
				// response in the next state.
				ticket, err = a.RegisterSidecar(ctx, ticket)
				if err != nil {
					log.Errorf("unable to register sidecar: %v", err)
					continue
				}
				...
				log.Infof("Creating reply mailbox for ticket=%v, "+
				...
				log.Infof("Sending registered ticket (id=%v) "+
				...
				if err != nil {
					log.Errorf("unable to convert ticket to str: %v", err)
					continue
				}
				// TODO(roasbeef): send msg reliability?
				err = a.client.SendCipherBoxMsg(
					ctx, providerStreamID, []byte(strTicket),
				)
				if err != nil {
			...
			case sidecar.StateRegistered:
				...
				ticket, err = sidecar.DecodeString(string(msg))
				if err != nil {
...
func (a *SidecarAcceptor) CoordinateSidecar(ctx context.Context,
	ticket *sidecar.Ticket, bid *order.Bid,
	...
	log.Infof("Creating mailbox for ticket=%x, w/ stream_id=%v",
		ticket.ID[:], streamID[:])
	...
	// From here, we'll launch a goroutine that will operate the staet
	...
	go func() {
		...
		for {
			...
			case sidecar.StateOffered:
				...
				// TODO(roasbeef): persists new ticket to disk?
				// * for restart case
			...
			case sidecar.StateRegistered:
				log.Infof("Submitting bid order for ticket=%v",
					ticket.ID[:])
			...
			case sidecar.StateOrdered:
				...
				log.Infof("Sending finalized ticket=%v to "+
